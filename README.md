...
•	Ayman Yehia
•	Mohammed Garrana 
•	Ahmed Mohamed Raafat
Under supervision of DR. Bahaa Hassan
05-12-2009
...
Table of Contents
1	Introduction	3
2	Document Walk through Guide	3
2.1	Document Structure	3
3	Basic concepts in cryptography	4
3.1	S-box	4
3.2	Feistel cipher	5
4	Blowfish	7
4.1	Introduction	7
4.2	Description of algorithm	8
4.2.1	Data- encryption	8
4.2.2	Key-expansion	12
5	Implementation of Blowfish using python	13
5.1	Flow Chart	13
5.1.1	Encryption Algorithm flowchart:	13
5.1.2	Function F flowchart:	14
5.2	Python code	16
6	Crypto analysis of Blowfish	42
7	Blowfish Applications.	43
8	References	44
...
3.1	S-box
In cryptography, an S-Box (Substitution-box) is a basic component of a symmetric key algorithm which performs substitution. In block ciphers, they are typically used to obscure the relationship between the key and the Cipher text.
In general, an S-Box takes some number of input “bits m”, and transforms them into some number of output “bits n”. An m×n S-Box can be implemented as a lookup table with 2^m  words of n-bits each. 
...
Given a 6-bit input, the 4-bit output is found by selecting the row using the outer two bits (the first and last bits), and the column using the inner four bits. 
...
3.2	Feistel cipher
•	Feistel networks were first seen commercially in IBM's Lucifer cipher, designed by Horst Feistel
•	The Feistel structure has the advantage that encryption and decryption operations are very similar, even identical in some cases, requiring only a reversal of the key schedule. Therefore the size of the code or circuitry required to implement such a cipher is nearly half. 
•	nature of the Feistel construction makes implementing the cryptosystem in hardware easier 
...
•	Let F be the round function and let be the sub-keys for the rounds   respectively.
•	Then the basic operation is as follows:
o	Split the plaintext block into two equal pieces, (L0, R0)
o	For each round   compute
o	 
o	 

•	Then the cipher text is (Rn + 1,Ln + 1).
•	Decryption of a cipher text (Rn + 1,Ln + 1) is accomplished by computing for   
•	  
•	 .
...
Blowfish uses a Feistel network, iterating a simple encryption function 16 times. The block size is 64 bits, and the key can be any length starting from 64 and up to 448 bits. Although there is a complex initialization phase required before any encryption can take place, the actual encryption of data is very efficient on large microprocessors
...
S3,0, S3,1,..., S3,255;
S4,0, S4,1,..,, S4,255.
The exact method used to calculate these subkeys will be described in section 4.2.2.
 

I.	Encryption process:

1-	Blowfish is a Feistel network consisting of 16 rounds (see Figure 1). The input is a 64-bit data of element, x.
2-	Divide x into two 32-bit halves: xL, xR For i = 1 to 16:
•	xL = LE0 XOR Pi
•	xR = F(xL) XOR RE0
•	Swap xL and xR
•	Then repeat it 16 time
...
1-	 Initialize first the P-array and then the four S-boxes, in order, with a fixed string. This string consists of the hexadecimal digits of pi (less the initial 3). For example:
		...
		P3 = 0x13198a2e
		P4 = 0x03707344
2-	split key 8 digit (64 bit ) to 2 part and xor first part sub key p1 , and second part with sub key p2, and so on until generate the new p18
3-	Until now p_boxes are Xor-ed with sec key however this not enough, because if  the p_boxes(which is known) are Xor-ed with new p_boxes , the secret key will be the result. 
4-	So we will generate new chain for p & s -boxs by encrypt them with p-box and old s-box using initial value xl=0  , xr=0.
5-	The output will be the new P1, P2 which will be the input to generate the ne P3,P4.
6-	And So on ,until get all new Ps 
7-	New P17,P18, then will be the initial value to re-generate s-box, and so on
8-	In total, 521 iterations are required to generate all required subkeys
521	= [(18+256*4)/2](where we have 18 P, and 4 S-boxs with256 element inside each) 
...
5	 Implementation of Blowfish using python
•	The following section contains the Blowfish implementation using python inside the border and the code flow chart. The code is written as an object oriented code.
•	We add comments as much as we can to make code clear to any one even if he doesn’t know python. Comments are started by “#” and with RED color.
•	 Code is origanly written by Michael Gilfix  (C) 2002 , and it’s open Source
•	The Code written by Michael Gilfix has a limitation of 64 bit ( 8 byte ) input to be encrypted 
•	We added 2 new functions, to support unlimited number of bytes as an input to cipher and unlimited cipher to decrypt.
•	We call the new 2 functions  (en), and (den) you can use them easy as following 
•	Blowfish('testtest').en('aaaaaaaabbbbbbbb')     --> to encrypt
•	Blowfish('testtest').den('z\xc4!O\x1d\x9fc\x1c\xa0A\x1e\xf99\xa7\x1b\xc9') --> to decrypt
...
5.1.1	Encryption Algorithm flowchart:


The following is a Flow Chart of the python implementation of BlowFish encryption algorithm
...
# 18 hex number to generate 18 different sub key
...
# 4 boxes in one list , each elemenet is list of 256 hex number
		self.s_boxes = [
			[
...
# Cycle through the p-boxes and round-robin XOR the key with the p-boxes
		# split key 8 digit (64 bit ) to 2 part and xor first part sub key p1 , and secand part with sub key p2, and so on untile generate th new p18
		# '\' to write same instruction in multi line
		# key divided to 2 part as following 1- take first char covert to unicode and shift left 24 bit , then char 2,3,4 but shift 16,8,0 2- xor them or add them
		key_len = len (key)
		index = 0
		...
		#untile now we xor p_boxes with sec key howevery this not enough ,because it i xor p_boxes(which is known) wiht new p_boxes i will get the key 
# so we will generate new chain for p & s -boxs by encrypte them with p-box and old s-box using initial value xl=0 ,xr=0.
#the output will be the new P1, P2 which will be the input to genrate the ne P3,P4.
#untile get P17,P18, then will be the initial value to re-genrate s-box, and so on
#In total, 521[(18+256*4)/2](where we have 18 P, and 4 S-boxs with256 element inside each) iterations are required to generate all required subkeys
		

# For the chaining process
		l, r = 0, 0

		# Begin chain replacing the p-boxes, increase i by 2 so loop 18/2= 9
...
def cipher (self, xl, xr, direction):
		...
		if direction == self.ENCRYPT:   # encryption
...
def __round_func (self, xl):
		...
		# Perform all ops as input is b bits  and out the last 32-bits to obtain the integer
#s_boxes[0][a] mean go to box 0 and get  element no a ,where a is only 8 bit so you can pick up from element 0 to element 256 which = to no of element in each box
	...
	# Use big endianess since that's what everyone else uses #Given a string of length one, return an integer representing the Unicode code
		# | --> or , <<  means shift
		# in following code creat xl , xr by 1- take char o and convert it to unicode then shift it binary by 24  2- do the same on  data 1 , 2,3 but by shifting them 16,8,0
		# 2- xor all part togother
		#3- do the sam 1,2 on data 4,5,6,7
...
6	Crypto analysis of Blowfish 
There is no effective cryptanalysis on the full-round version of Blowfish known publicly as of 2009. A sign extension bug in one publication of C code has been identified.
In 1996, Serge Vaudenay found a known-plaintext attack requiring 28r + 1 known plaintexts to break, where r is the number of rounds. 
Moreover, he also found a class of weak keys that can be detected and broken by the same attack with only 24r + 1 known plaintexts. This attack cannot be used against the regular Blowfish; it assumes knowledge of the key-dependent S-boxes. 
Vincent Rijmen, in his Ph.D. thesis, introduced a second-order differential attack that can break four rounds and no more. There remains no known way to break the full 16 rounds, apart from a brute-force search
Bruce Schneier notes that while Blowfish is still in use, he recommends using the more recent Twofish algorithm instead
  

7	Blowfish Applications.
This list below is a compilation of some encryption products that use the Blowfish encryption algorithm 
1-	96Crypt by fever.link
A file and folder encryption/decryption program.
2-	A-Lock by Trillium Technology Group
Encryption software that integrates with popular Windows e-mail programs.
3-	Access Manager by Citi-Software Ltd.
A password manager for Windows. Free for personal use.
4-	AEdit
A free Windows word processor incorporating text encryption.
5-	Answerstream.com by Logivox Software, Inc.
An online survey management system developed by Logivox for Howard Merrell & Partners.
6-	AntiSnoop Password Dropper
A password database designed to protect against keystroke loggers, mouse monitors,..
7-	Ashampoo Security Manager by Ashampoo GmbH & Co. KG
One-click encryption and decryption with self-decrypting archives.
8-	Avactis Shopping Cart
E-commerce software. Uses Blowfish to encrypt credit card and cardholder data.
9-	Blowfish.NET by Markus Hahn
Brings the Blowfish algorithm to the Microsoft .NET platform. Open source.
